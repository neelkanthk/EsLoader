...
class Aws
{
    ...
    $region, callable $credentialProvider = null, callable $wrappedHandler = null
    )
    {
        $this->signer = new SignatureV4('es', $region);
        $this->wrappedHandler = $wrappedHandler ?: ClientBuilder::defaultHandler();
        $this->credentialProvider = $credentialProvider ?: CredentialProvider::defaultProvider();
    }
    ...
    private function createPsr7Request(array $ringPhpRequest)
    {
        ...
        $ringPhpRequest['headers'][$hostKey][0] = parse_url($ringPhpRequest['headers'][$hostKey][0])['host'];
        ...
        return new Request(
                $ringPhpRequest['http_method'], $uri, $ringPhpRequest['headers'], $ringPhpRequest['body']
        );
    }
